...
class Dataset(object):
    ...
    def __init__(self, feature, data_dir=config.data_dir, nhwc=config.nhwc, num_threads=config.num_threads, capacity=config.capacity, shuffle=config.shuffle):
        ...
        with open(os.path.join(self.data_dir, 'list_attr_celeba_test.txt'), 'r') as f:
    ...
    @property
    def filenames1(self):
        path = "datasets/celebA/align_test"
        files = os.listdir(path)
        filenames = [os.path.join(self.data_dir, 'align_test/{:06d}.jpg'.format(idx+1)) for idx in self.idxs1]
        
        file_need = []
        for file in files:
            file_p = '/notebooks/NTU/GeneGAN/./datasets/celebA/align_test/'+str(file)
            for file_filter in filenames:
                if file_p == file_filter:
                    file_need.append(file_filter)
    ...
    def filenames2(self):
        path = "datasets/celebA/align_test"
        files = os.listdir(path)
        filenames = [os.path.join(self.data_dir, 'align_test/{:06d}.jpg'.format(idx+1)) for idx in self.idxs2]
